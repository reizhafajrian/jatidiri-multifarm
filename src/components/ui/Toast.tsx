...
export function Toast({ visible, className, ...props }: ToastProps) {
  return (
    <div
      className={cn(
        "min-h-16 mb-2 flex min-w-[350px] flex-col items-start gap-1 rounded-md bg-white px-6 py-4 font-semibold shadow-lg",
        visible && "animate-in slide-in-from-top-5",
        className
      )}
      {...props}
    />
    ...
    ({ visible }) => (
      <Toast
        ...
        className={cn({
          ...
          "bg-success-1 text-white": type === "success",
        })}
      >
